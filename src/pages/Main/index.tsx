...
export default function Main() {
  ...
  const pokemons = useSelector(
    (state: { pokemon: { list: { name: string }[] } }) => state.pokemon.list
  )
  const loading = useSelector(
    (state: { pokemon: { loading: boolean } }) => state.pokemon.loading
  )
    ...
    <>
      ...
      <main className={`list ${loading && "list--loader"}`}>
        ...
        {!loading &&
          pokemons.map((el) => (
            <Link
              className="list__element pokemons"
              key={el.name}
              to={`/pokemon/${el.name}`}
            >
              <picture>
                <source
                  srcSet="https://via.placeholder.com/300.webp"
                  type="image/webp"
                />
                <source
                  srcSet="https://via.placeholder.com/300.jpeg"
                  type="image/jpeg"
                />
                <img
